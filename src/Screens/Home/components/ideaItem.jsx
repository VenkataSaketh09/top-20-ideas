    ...
    <div className='my-5 border shadow-lg rounded-lg p-5'>
      <div className=''>
        ...
        <div className='flex flex-row items-center'>
          <h2
            className={`text-lg hover:bg-slate-600 rounded-md p-1 cursor-pointer ${checkIsAlreadyupVoted(idea?.id) && 'bg-slate-700'}`}
            onClick={() => upVotehandler()}
          >🔥</h2>
          <h2 className='text-lg hover:bg-slate-600 rounded-md p-1 font-bold'>{idea?.vote}</h2>
          <h2
            className={`text-lg hover:bg-slate-600 rounded-md p-1 cursor-pointer ${checkIsAlreadydownVoted(idea?.id) && 'bg-slate-700'}`}
